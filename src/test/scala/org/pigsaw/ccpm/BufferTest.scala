...
class BufferTest extends FlatSpec with Matchers {
  ...
  "make" should "set the duration to half the path length if there's time (1)" in {
    val t0 = Task('t0, 5)
    val t1 = Task('t1, 6)
    val t2 = Task('t2, 7)
    val b = Buffer.make('b0, Seq(t0, t1, t2), 100)
    b.duration should equal (5+6+7)
  }
  ...
  it should "set the duration to the given maximum if the path is too long" in {
    ...
    val t1 = Task('t1, 40)
    val t2 = Task('t2, 50)
    val b = Buffer.make('b0, Seq(t0, t1, t2), 60)
    b.duration should equal (60)
  }
